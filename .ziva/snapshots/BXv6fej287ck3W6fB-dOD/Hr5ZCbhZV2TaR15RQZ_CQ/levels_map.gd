...
func _ready() -> void:
	...
	# Create a label to show level names
	...
	level_label.add_theme_constant_override("outline_size", 8)
	...
	for child in level_buttons.get_children():
		if child is TextureButton:
			...
			# Add a permanent label below the button to make it visible
			var lbl := Label.new()
			...
			lbl.position.y = button.size.y + 5
...
func _on_level_button_hovered(button: TextureButton) -> void:
	...
	# Show level name
	level_label.text = button.name
	level_label.show()
	level_label.global_position = marker.global_position + Vector2(-level_label.size.x / 2.0, -60)
